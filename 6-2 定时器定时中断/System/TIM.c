...
void TIM_Init(void)
{
	RCC_APB1PeriphClockCmd(RCC_APB1Periph_TIM3, ENABLE); //干啥首先要开启外设时钟
	
	TIM_InternalClockConfig(TIM3); //将定时器的钟源显示配置为内部时钟（不过默认就是内部，其实没用这一步也行）
	...
	TIM_TimeBaseInitStruct.TIM_Period = 10000 - 1; //设置ARR的自动重装值
	TIM_TimeBaseInitStruct.TIM_Prescaler = 7200 - 1; //设置分频系数（不过实际分频系数是PSC + 1）
	...
	TIM_TimeBaseInit(TIM3, &TIM_TimeBaseInitStruct); //Init之后才算是写入寄存器生效了
	
	/*!!!!!Init之后会自动触发一个更新事件，导致中断标志位SET，使得中断一开始就触发，因此下一步要Claer中断标志位!!!!!*/
	
	TIM_ClearFlag(TIM3, TIM_FLAG_Update);
	
	TIM_ITConfig(TIM3, TIM_IT_Update, ENABLE); //这个是用来使能/禁用一个定时器（TIM）的中断源的
	...
	TIM_Cmd(TIM3, ENABLE); //最后启动，防止没配置完中途启动发生错误
